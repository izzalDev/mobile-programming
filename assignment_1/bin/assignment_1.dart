...
void main() {
  ...
  print('\n4. Apply Discount (10%):');
  print('Subtotal: Rp ${cart.calculateSubtotal()}');
  print(
    'Discount (10%): -Rp ${cart.calculateDiscount(DiscountType.percentage, 0.1).toStringAsFixed(0)}',
  );
  print(
    'Total: Rp ${cart.calculateTotal(DiscountType.percentage, 0.1).toStringAsFixed(0)}',
  );

  print('\n5. Validation:');
  if (cart.validateCart(catalog)) {
    print('✓ Semua produk tersedia di stok');
  }
  if (cart.getValidationErrors(catalog).isEmpty) {
    print('✓ Cart valid untuk checkout');
  }
  // print('\n================= KERANJANG AWAL =================');
  // cart.displayCart();
  //
